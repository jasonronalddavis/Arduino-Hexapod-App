...
void customAudio::Record() {
    ...
    // After recording, encode the data to base64
    EncodeToBase64();
}

// Function to encode WAV data into base64 format and store it in the content attribute
void customAudio::EncodeToBase64() {
    // Encode header
    content = base64::encode(paddedHeader, sizeof(paddedHeader));
    content.replace("\n", "");  // Remove any newline characters

    // Encode wavData chunks
    for (int j = 0; j < wavDataSize / dividedWavDataSize; ++j) {
        String encodedChunk = base64::encode((byte*)wavData[j], dividedWavDataSize);
